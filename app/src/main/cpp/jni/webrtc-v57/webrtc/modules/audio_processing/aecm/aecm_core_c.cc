...
static int absFrequency(ComplexInt16* freq_signal,
...
/*
//yangrui 0814
static void ComputeCoherenceM(const CoherenceStateM* coherence_state,
                             float* cohde,
                             float* cohxd,
                             float* cohxe) {
  // Subband coherence
...
static void UpdateCoherenceSpectra(int mult,
                                   ...
                                   ComplexInt16 efw[PART_LEN],
                                   ComplexInt16 dfw[PART_LEN],
                                   ComplexInt16 xfw[PART_LEN],
                                   CoherenceStateM* coherence_state) {
  ...
  for (i = 0; i < PART_LEN; i++) {
    coherence_state->sd[i] =
        ...
        ptrGCoh[1] * (dfw[i].real * dfw[i].real + dfw[i].imag * dfw[i].imag);
    coherence_state->se[i] =
        ptrGCoh[0] * coherence_state->se[i] +
        ptrGCoh[1] * (efw[i].real * efw[i].real + efw[i].imag * efw[i].imag);
    ...
    coherence_state->sx[i] =
        ...
        ptrGCoh[1] *
            WEBRTC_SPL_MAX(xfw[i].real * xfw[i].real + xfw[i].imag * xfw[i].imag,
                           WebRtcAec_kMinFarendPSD);

    coherence_state->sde[i][0] =
        ptrGCoh[0] * coherence_state->sde[i][0] +
        ptrGCoh[1] * (dfw[i].real * efw[i].real + dfw[i].imag * efw[i].imag);
    coherence_state->sde[i][1] =
        ptrGCoh[0] * coherence_state->sde[i][1] +
        ptrGCoh[1] * (dfw[i].real * efw[i].imag - dfw[i].imag * efw[i].real);

    coherence_state->sxd[i][0] =
        ptrGCoh[0] * coherence_state->sxd[i][0] +
        ptrGCoh[1] * (dfw[i].real * xfw[i].real + dfw[i].imag * xfw[i].imag);
    coherence_state->sxd[i][1] =
        ptrGCoh[0] * coherence_state->sxd[i][1] +
        ptrGCoh[1] * (dfw[i].real * xfw[i].imag - dfw[i].imag * xfw[i].real);

    coherence_state->sxe[i][0] =
        ptrGCoh[0] * coherence_state->sxe[i][0] +
        ptrGCoh[1] * (efw[i].real * xfw[i].real + efw[i].imag * xfw[i].imag);
    coherence_state->sxe[i][1] =
        ptrGCoh[0] * coherence_state->sxd[i][1] +
        ptrGCoh[1] * (efw[i].real * xfw[i].imag - efw[i].imag * xfw[i].real);
...
int WebRtcAecm_ProcessBlock(AecmCore* aecm,
  ...
  //yangrui 0806
  ComplexInt16* xfw = (ComplexInt16*)(((uintptr_t)xfw_buf + 31) & ~31);
  ...
  int32_t  sumDE = 0;
  int32_t  sumXE = 0;
//  float cohde[PART_LEN], cohxd[PART_LEN], cohxe[PART_LEN];
...
//yangrui 0806 coherence
 absFrequency(efw,efa,&efaSum);

/*
//yangrui 0814
  UpdateCoherenceSpectra(aecm->mult, 1,efw, dfw, xfw, &aecm->coherence_state);
  ComputeCoherenceM(&aecm->coherence_state, cohde, cohxd, cohxe);
    const int prefBandSize = kPrefBandSize / aecm->mult;
    const int minPrefBand = 4 / aecm->mult;
    hNlXdAvg = 0;
    hNlXeAvg = 0;
    hNlDeAvg = 0;
...
/*
  ...
  uint32_t dtemp = 0;
    ...
    for (i = 1; i < PART_LEN; i++)
    {
        ...
        sumXE += (xtemp - xfaSum/PART_LEN) * (etemp - efaSum/PART_LEN);
        sumXD += (xtemp - xfaSum/PART_LEN) * (dtemp - dfaNoisySum/PART_LEN);
        sumDE += (etemp - efaSum/PART_LEN) * (dtemp - dfaNoisySum/PART_LEN);
        sumXX += (xtemp - xfaSum/PART_LEN) * (xtemp - xfaSum/PART_LEN);
        sumEE += (etemp - efaSum/PART_LEN) * (etemp - efaSum/PART_LEN);
        sumDD += (dfaNoisy[i] - dfaNoisySum/PART_LEN) * (dfaNoisy[i] - dfaNoisySum/PART_LEN);
    }
  //  LOGI("PART_LEN = %d, sumXE = %d, sumXD = %d, sumDE = %d, sumXX = %d, sumEE = %d, sumDD = %d",PART_LEN, sumXE, sumXD, sumDE, sumXX, sumEE, sumDD);
       ...
       if(sumDD > 0 &&  sumEE > 0)
            hNlDeAvg = sumDE *1000 / WebRtcSpl_Sqrt(sumDD * sumEE );
        if(sumDD >0 && sumXX > 0)
            hNlXdAvg = sumXD *1000 / WebRtcSpl_Sqrt(sumDD * sumXX );
        if(sumXX > 0 && sumEE > 0)
            hNlXeAvg = sumXE *1000 /WebRtcSpl_Sqrt(sumEE * sumXX );
*/


    WebRtcSpl_CrossCorrelation((int32_t*)&hNlXeAvg,(int16_t*)xfa,(int16_t*)efa,PART_LEN,1,16,1);
    WebRtcSpl_CrossCorrelation((int32_t*)&hNlXdAvg,(int16_t*)xfa,(int16_t*)dfaNoisy,PART_LEN,1,16,1);
    WebRtcSpl_CrossCorrelation((int32_t*)&hNlDeAvg,(int16_t*)dfaNoisy,(int16_t*)efa,PART_LEN,1,16,1);

     if(aecm->totCount == 1) {
            aecm->hNlXdMean = hNlXdAvg*1000;
            aecm->hNlXeMean = hNlXeAvg*1000;
            aecm->hNlDeMean = hNlDeAvg*1000;
        }
   if(aecm->totCount > 0 && aecm->totCount < 4000) {
        aecm->hNlXdMean = (aecm->hNlXdMean *(aecm->totCount -1)  + hNlXdAvg*1000 ) /aecm->totCount;
        aecm->hNlXeMean = (aecm->hNlXeMean *(aecm->totCount -1)  + hNlXeAvg*1000 ) /aecm->totCount;
        aecm->hNlDeMean = (aecm->hNlDeMean *(aecm->totCount -1)  + hNlDeAvg*1000 ) /aecm->totCount;
       if(aecm->hNlXeMean > 0){
            aecm->hAecRatio = aecm->hNlXdMean *1000 /aecm->hNlXeMean ;
       }
      //   LOGI("totCount = %d, hNlXdAvg = %d,hNlXdMean = %d, hNlXeAvg = %d, hNlXeMean = %d, hAecRatio = %d", aecm->totCount,hNlXdAvg,aecm->hNlXdMean, hNlXeAvg,aecm->hNlXeMean, aecm->hAecRatio);
