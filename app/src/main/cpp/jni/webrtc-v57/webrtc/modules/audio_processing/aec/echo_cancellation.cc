...
int32_t WebRtcAec_Process(void* aecInst,
  ...
  if (WebRtcAec_extended_filter_enabled(aecpc->aec)) {
    ...
    //yangrui 0814
    xdCoherence = (int) (aecpc->aec->hNlXdMean *1000);
    xeCoherence = (int) (aecpc->aec->hNlDeMean *1000);
    aecRatio = (int) (aecpc->aec->hAecRatio * 1000);
...
int WebRtcAec_set_config(void* handle, AecConfig config) {
  ...
  //yangrui
  //LOGI("%s_%d, metricsMode=: %d",__FILE__,__LINE__,config.metricsMode);
...
static int ProcessNormal(Aec* aecpc,
 ...
 //  memset(&metrics, 0, sizeof(metrics));
  ...
  if (aecpc->startup_phase) {
    ...
    //yangrui
   //  WebRtcAec_GetMetrics(aecpc->aec, &metrics);
     // ERLE
...
static void ProcessExtended(Aec* self,
...

  {
    ...
    WebRtcAec_ProcessFrames(self->aec, near, num_bands, num_samples,
        ...
        //yangrui
        // WebRtcAec_GetMetrics(self->aec, &metrics);
         // ERLE
         //   LOGI("ERLE.instant=: %d, ERLE.average=: %d",metrics.erle.average,metrics.erle.instant);
  }
}
