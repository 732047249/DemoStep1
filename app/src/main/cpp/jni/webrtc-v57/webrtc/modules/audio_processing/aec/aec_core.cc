...
float wfb[2][kExtendedNumPartitions * PART_LEN1];
FILE *dbgFp3;
...
static void UpdateCoherenceSpectra(int mult,
  ...
  int i;
  float sdSum = 0, seSum = 0 , sxSum = 0;

  float seHigh = 0, seLow = 0;
  ...
  for (i = 0; i < PART_LEN1; i++) {
    ...
    //yangrui
    coherence_state->sxe[i][0] =
            ptrGCoh[0] * coherence_state->sxe[i][0] +
            ptrGCoh[1] * (efw[0][i] * xfw[0][i] + efw[1][i] * xfw[1][i]);
        coherence_state->sxe[i][1] =
            ptrGCoh[0] * coherence_state->sxe[i][1] +
            ptrGCoh[1] * (efw[0][i] * xfw[1][i] - efw[1][i] * xfw[0][i]);

    sdSum += coherence_state->sd[i];
    seSum += coherence_state->se[i];
    sxSum += coherence_state->sx[i];

    

    if(i >= 25 && i < 41)
    {
      seLow += coherence_state->se[i];
    }

    if(i >= 41 && i < 57)
    {
      seHigh += coherence_state->se[i];
    }
  }
//printf("%s#%d, TRACE\n", __FILE__,__LINE__);
//yangrui
  coherence_state->sumSd = sdSum;
  coherence_state->sumSe = seSum;
  coherence_state->sumSx = sxSum;
  coherence_state->seLow = seLow;
  coherence_state->seHigh = seHigh;
//FileWriteFloat(dbgFp1, sxLow);
//FileWriteFloat(dbgFp2, sxHigh);
//FileWriteFloat(dbgFp3, sxHigh/sxLow);
//printf("%s#%d, TRACE, sxLow = %f\n", __FILE__,__LINE__,sxLow);
...
static void ComputeCoherence(const CoherenceState* coherence_state,
  ...
  for (int i = 0; i < PART_LEN1; i++) {
    ...
    /*
    cohxe[i] = (coherence_state->sxe[i][0] * coherence_state->sxe[i][0] +
                    coherence_state->sxe[i][1] * coherence_state->sxe[i][1]) /
                   (coherence_state->sx[i] * coherence_state->se[i] + 1e-10f);
    */
  }
}
...
static void UpdateMetrics(AecCore* aec) {
  ...
  if (aec->farlevel.averagelevel.EndOfBlock()) {
    ...
    if ((aec->stateCounter > (0.5f * kCountLen * kSubCountLen)) &&
      ...
      LOGI("far_average_level=: %f, near_average_level=: %f, nlpout_average_level=: %f",far_average_level,near_average_level,nlpout_average_level);
       LOGI("aec->erle.instant=: %f, aec->erle.average=: %f",aec->erle.instant,aec->erle.average);
    }
...
static void FormSuppressionGain(AecCore* aec,
                                ...
                                float hNl[PART_LEN1]) {
  float hNlDeAvg, hNlXdAvg, hNlXeAvg;
...
/*
 ...
 hNlXeAvg = 0;
  ...
  hNlXeAvg /= prefBandSize;
  hNlXeAvg = 1 - hNlXeAvg;
  */
    //yangrui  0804 coherence
  //  clock_t start = clock();
    if(aec->frame_count == 1) {
       aec->hNlXdMean = hNlXdAvg;
       // aec->hNlXeMean = hNlXeAvg;
       aec->hNlDeMean = hNlDeAvg;
    }
    if(aec->frame_count > 0 && aec->frame_count < 4000) {
      aec->hNlXdMean = (aec->hNlXdMean * (aec->frame_count - 1) + hNlXdAvg) / aec->frame_count;
     // aec->hNlXeMean = (aec->hNlXeMean * (aec->frame_count - 1) + hNlXeAvg) / aec->frame_count;
      aec->hNlDeMean = (aec->hNlDeMean * (aec->frame_count - 1) + hNlDeAvg) / aec->frame_count;
      aec->hAecRatio = aec->hNlXdMean / (aec->hNlDeMean + 1e-6f);
    //  LOGI("frame_count = %d, hNlXdAvg = %f,hNlXdMean = %f, hNlDeAvg = %f, hNlDeMean = %f, hAecRatio = %f",
    //  aec->frame_count,hNlXdAvg,aec->hNlXdMean, hNlDeAvg,aec->hNlDeMean, aec->hAecRatio);
    }
    if(hNlDeAvg > 0.2 && aec->frame_count < 1000) {
      aec->coherence_state.cohedMean = hNlDeAvg + 0.4;
    }
	if(hNlDeAvg > 0.3) {
      aec->coherence_state.cohedMean = hNlDeAvg-0.1;
    }
  ...
  hNlFbLow = 1;//hNlDeAvg;
 // for (int i = 0; i < PART_LEN1; ++i) {
 //     hNl[i] = WEBRTC_SPL_MIN(cohde[i]*(1-cohxe[i])*1.1, 1);
 //     }
  ...
  if (hNlDeAvg > 0.9f && hNlXdAvg > 0.9f) {
    aec->stNearState = 1;
    aec->suppState = 0;
    aec->dtState = 0;
  } else if (hNlDeAvg < 0.85f || hNlXdAvg < 0.75f) {
    aec->stNearState = 0;
    aec->suppState = 1;
  }
  ...
  if (aec->hNlXdAvgMin > 0.9) {
    aec->echoState = 0;
    aec->overDrive = 2; //min_overdrive[aec->nlp_mode];
      for (int i = 0; i < PART_LEN1; ++i) {
        //hNl[i] = WEBRTC_SPL_MIN((1 - cohxd[i])*(1-cohxe[i])*1.1,1);
		//hNl[i] = WEBRTC_SPL_MAX(1 - cohxd[i]-cohxe[i],0);
    hNl[i] = WEBRTC_SPL_MAX(1 - cohxd[i],0.05);
	//hNl[i] = WEBRTC_SPL_MIN((1 - cohxd[i])*powf(4,0.4-cohxe[i]),0.95);
      }
      hNlFb = 0.95;//hNlXdAvg;
      hNlFbLow = 0.95;//hNlXdAvg;
     // aec->dtState = 0;   
  }
  ...
  if (aec->suppState == 0) {
      memcpy(hNl, cohde, sizeof(hNl[0]) * PART_LEN1);
      for (int i = 0; i < PART_LEN1; ++i) {
      //hNl[i] = WEBRTC_SPL_MIN(cohde[i]*powf(4,0.4-cohxe[i]), 1);
	  //hNl[i] = WEBRTC_SPL_MAX(cohde[i]-cohxe[i]/2,0);
    hNl[i] = WEBRTC_SPL_MIN(cohde[i] ,0.95);
      }
      hNlFb = 0.95;//hNlDeAvg;
      hNlFbLow = 0.95;//hNlDeAvg;
      aec->dtState = 0;
      aec->overDrive = 2;
  } else {
    ...
    //      hNl[i] /=4;
      ...
      hNlFb = hNlXdAvg;
      hNlFbLow = hNlXdAvg;
     // hNlFb = WEBRTC_SPL_MIN(hNlXdAvg,hNlDeAvg);
     // hNlFbLow = WEBRTC_SPL_MIN(hNlXdAvg,hNlDeAvg);
      aec->overDrive = min_overdrive[aec->nlp_mode];
      /*
      if(aec->dtStatePre == 1)
      {
        for (int i = 0; i < PART_LEN1; ++i) {
          hNl[i] = WEBRTC_SPL_MIN((1 - cohxd[i])+0.05, 0.95);
        }
        aec->overDrive = 2.2;
      }
        */  
  ...
  if (aec->coherence_state.seHigh > 8e6 &&  aec->suppState == 1 && hNlDeAvg < 0.6){
      aec->dtState = 1;
  } 

  if (aec->suppState == 1 && hNlDeAvg > 0.95){
      aec->dtState = 0;
  } 
/*
    if (aec->dtState == 1) {
      ...
      hNlFb = WEBRTC_SPL_MIN(WEBRTC_SPL_MAX(hNlDeAvg * 2, hNlXdAvg * 2),0.95);
      hNlFbLow = WEBRTC_SPL_MIN(WEBRTC_SPL_MAX(hNlDeAvg * 2, hNlXdAvg * 2),0.95);
      aec->overDrive = 2;
    } 

*/

//FileWriteFloats(dbgFp1,hNl,PART_LEN1);
  ...
  WebRtcAec_Overdrive(aec->overdrive_scaling, hNlFb, hNl);
    if(aec->dtState == 1){
      ...
      //  memcpy(aec->wfBuf, aec->wfBufPre, sizeof(float)* 2 * kExtendedNumPartitions * PART_LEN1);
      /*
	    for (int i = 0; i < PART_LEN1; ++i) {
        hNl[i] *= 1.1;
        if (hNl[i] > 0.95)
...
static void EchoSuppression(const OouraFft& ooura_fft,
  ...
  float cohde[PART_LEN1], cohxd[PART_LEN1], cohxe[PART_LEN1];
  float hNl[PART_LEN1];
...
static void ProcessNearendBlock(
  ...
  ///* BEGIN MODIFICATIONS BY YEECALL *///
  ...
  if (aec->delay_logging_enabled) {
    if (WebRtc_AddFarSpectrumFloat(aec->delay_estimator_farend,
      ...
      ESTIMATED_DELAY = delay_estimate * 4; // as milliseconds
...
void WebRtcAec_FreeAec(AecCore* aec) {
  ...
  dbgFp3 = fopen("/data/data/com.meicai.im.app/wfb.dat", "wb");
  if(dbgFp3) {
    LOGI("write wfb to file");
    memcpy(wfb, aec->wfBuf, sizeof(complex_t) * kExtendedNumPartitions * PART_LEN1);
    fwrite((float *)wfb, sizeof(complex_t) * kExtendedNumPartitions * PART_LEN1, 1, dbgFp3);
    fclose(dbgFp3);
  }
...
int WebRtcAec_InitAec(AecCore* aec, int sampFreq) {
  ...
  memset(wfb, 0, 2 * kExtendedNumPartitions * PART_LEN1);

  dbgFp3 = fopen("/data/data/com.meicai.im.app/wfb.dat", "rb");
  if(dbgFp3){
    LOGI("load wfb from file");
    int nRet = fread(wfb, sizeof(float), sizeof(complex_t) * kExtendedNumPartitions * PART_LEN1, dbgFp3);
    fclose(dbgFp3);
  }
  memcpy(aec->wfBuf, wfb, sizeof(complex_t) * kExtendedNumPartitions * PART_LEN1);
...
//yangrui 0814
int WebRtcAec_XDCoherence(AecCore* self) {
//LOGI("%s_%d, hNlXdMean=: %f",__FILE__,__LINE__, self->hNlXdMean);
  return (int) (self->hNlXdMean*1000);
}
int WebRtcAec_XECoherence(AecCore* self) {
  return (int) (self->hNlDeMean*1000);
}
int WebRtcAec_AECRatio(AecCore* self) {
  return (int) (self->hAecRatio*1000);
}
